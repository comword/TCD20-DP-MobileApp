...
void Camera::printCamProps(const char *id)
{
    // exposure range
...
std::string Camera::getFacingCamId(Facing facing)
{
    ...
    for (int i = 0; i < cameraIds->numCameras; ++i)
    {
        ...
        auto camFacing = static_cast<acamera_metadata_enum_android_lens_facing_t>(
                lensInfo.data.u8[0]);

        if (camFacing == static_cast<acamera_metadata_enum_acamera_lens_facing>(facing))
        {
            targetId = id;
            break;
        }
